"""Authenticated role enforcement for the FI Ops portal boundary.
...
class ConfigTokenAuthenticator:
    ...
    Tokens are configured via ``FI_OPS_AUTH_TOKENS`` as a comma-separated
    ``<token>:<role1|role2>`` map, e.g.::

        FI_OPS_AUTH_TOKENS="dev-ops-token:FI_OPS|ADMIN,dev-readonly:FI_OPS"
    ...
    def __init__(self, token_map: dict[str, frozenset[str]] | None = None) -> None:
        if token_map is not None:
            ...
            self._tokens = _parse_token_map(os.getenv("FI_OPS_AUTH_TOKENS", ""))

  ...
  return (
    <div className="min-h-screen bg-[#f4f7f6] text-slate-900">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-10 flex h-20 items-center justify-between border-b border-slate-200 bg-[#f4f7f6]/95 px-5 backdrop-blur md:px-8">
          <div className="flex items-center gap-4"><button className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm lg:hidden" onClick={() => setOpen(true)} aria-label="Open navigation">Menu</button>{searchOpen ? <input autoFocus className="w-52 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-teal-600 md:w-80" placeholder="Search transactions, customers..." onBlur={() => setSearchOpen(false)} /> : <button className="text-sm text-slate-500 hover:text-slate-900" onClick={() => setSearchOpen(true)}>Search <span className="ml-2 rounded border border-slate-300 bg-white px-1.5 py-0.5 text-xs">Ctrl K</span></button>}</div>
          <div className="flex items-center gap-5 text-sm"><Link to="/bank/notifications" className="relative text-slate-500 hover:text-slate-900">Notifications{unread > 0 && <span className="absolute -right-2 -top-2 grid h-4 min-w-4 place-items-center rounded-full bg-red-600 px-1 text-[10px] text-white">{unread}</span>}</Link><button className="hidden text-slate-500 hover:text-slate-900 md:block">Help</button><div className="hidden h-6 w-px bg-slate-300 md:block" /><div className="text-right"><p className="font-semibold">Avery Morgan</p><p className="text-xs text-slate-500">Risk Manager</p></div><div className="grid h-9 w-9 place-items-center rounded-full bg-[#102a2a] text-xs font-bold text-teal-200">AM</div></div>
        </header>
        {children}
      </div>
    </div>

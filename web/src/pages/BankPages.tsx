...
function OperationalCard({ title, value, detail, link }: { title: string; value: string; detail: string; link: string }) { return <Link to={link} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm hover:border-teal-400"><p className="text-sm font-semibold text-slate-500">{title}</p><p className="mt-3 font-semibold">{value}</p><p className="mt-1 text-xs text-slate-500">{detail}</p><span className="mt-4 block text-xs font-bold text-teal-700">Open workspace -&gt;</span></Link>; }
...
export function AnalyticsPage() { return <div className="mx-auto max-w-7xl px-6 py-8"><PageTitle eyebrow="Intelligence" title="Risk Analytics" detail="Measure model outcomes and fraud patterns across the portfolio." action={<select className="rounded-md border border-slate-300 bg-white px-3 py-2.5 text-sm"><option>Last 30 days</option><option>Last 7 days</option><option>Last 90 days</option></select>} /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Kpi label="Average risk score" value="34.8" change="-2.4% vs prior period" tone="teal" /><Kpi label="Fraud rate" value="5.16%" change="+0.4 pts" tone="amber" /><Kpi label="False positive rate" value="2.8%" change="-0.6 pts" tone="teal" /><Kpi label="Verification rate" value="7.7%" change="+1.1 pts" tone="amber" /></div><div className="mt-6 grid gap-6 lg:grid-cols-2"><ChartPanel title="Risk score distribution"><BarChart values={[72, 18, 8, 2]} labels={['Low', 'Medium', 'High', 'Critical']} /></ChartPanel><ChartPanel title="Decision mix"><BarChart values={[72, 20, 8]} labels={['Allow', 'Verify', 'Block']} /></ChartPanel><ChartPanel title="Fraud by transaction type"><BarChart values={[62, 24, 14]} labels={['Purchase', 'Transfer', 'Withdrawal']} /></ChartPanel><ChartPanel title="Fraud by merchant category"><BarChart values={[48, 28, 16, 8]} labels={['E-commerce', 'Travel', 'Retail', 'Digital goods']} /></ChartPanel></div></div>; }
function ChartPanel({ title, children }: { title: string; children: ReactNode }) { return <Panel className="p-5"><h2 className="font-semibold">{title}</h2><p className="mt-1 text-xs text-slate-500">Relative portfolio share</p><div className="mt-6">{children}</div></Panel>; }
function BarChart({ values, labels }: { values: number[]; labels: string[] }) { return <div className="space-y-4">{values.map((value, index) => <div key={labels[index]}><div className="mb-1 flex justify-between text-xs"><span>{labels[index]}</span><strong>{value}%</strong></div><div className="h-3 rounded-full bg-slate-100"><div className={`h-3 rounded-full ${index === values.length - 1 ? 'bg-red-500' : index === values.length - 2 ? 'bg-amber-400' : 'bg-teal-600'}`} style={{ width: `${value}%` }} /></div></div>)}</div>; }
...
export function MerchantsPage() { return <div className="mx-auto max-w-7xl px-6 py-8"><PageTitle eyebrow="Monitoring" title="Merchants" detail="Monitor merchant exposure, fraud rate, and authorization outcomes." /><Panel className="overflow-hidden"><table className="min-w-[850px] w-full text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-[0.12em] text-slate-500"><tr>{['Merchant', 'Category', 'Volume', 'Transactions', 'Fraud rate', 'Avg risk', 'Block rate', 'Status'].map((heading) => <th key={heading} className="px-5 py-3 font-bold">{heading}</th>)}</tr></thead><tbody>{merchants.map((item) => <tr key={item.id} className="border-t border-slate-100"><td className="px-5 py-4"><p className="font-semibold">{item.name}</p><p className="text-xs text-slate-500">{item.id}</p></td><td className="px-5 py-4">{item.category}</td><td className="px-5 py-4 font-semibold">{item.volume}</td><td className="px-5 py-4">{item.transactions.toLocaleString()}</td><td className="px-5 py-4"><Pill value={item.fraudRate} /></td><td className="px-5 py-4">{item.score}</td><td className="px-5 py-4">{item.blockRate}</td><td className="px-5 py-4"><Pill value={item.status} /></td></tr>)}</tbody></table></Panel></div>; }

export function PoliciesPage() { const [allow, setAllow] = useState(29); const [verify, setVerify] = useState(79); const [block, setBlock] = useState(80); const [reason, setReason] = useState(''); const [saved, setSaved] = useState(false); return <div className="mx-auto max-w-5xl px-6 py-8"><PageTitle eyebrow="Controls" title="Fraud Policies" detail="Manage decision thresholds with an auditable confirmation step." /><Panel className="p-6"><div className="flex items-start justify-between"><div><h2 className="text-lg font-semibold">Default authorization policy</h2><p className="mt-1 text-sm text-slate-500">Version 1.2 · Last updated by Risk Manager</p></div><Pill value="ENABLED" /></div><div className="mt-6 grid gap-4 md:grid-cols-3"><PolicyInput label="Allow maximum score" value={allow} onChange={setAllow} /><PolicyInput label="Verification maximum score" value={verify} onChange={setVerify} /><PolicyInput label="Block minimum score" value={block} onChange={setBlock} /></div><div className="mt-6 rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>Threshold changes affect live authorization behavior.</strong><p className="mt-1">Every change requires a reason and is recorded in Audit Logs.</p></div><label className="mt-6 block text-sm font-semibold">Reason for change<input value={reason} onChange={(event) => setReason(event.target.value)} className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2.5 font-normal" placeholder="Describe why this policy is changing" /></label><button disabled={!reason.trim()} onClick={() => setSaved(true)} className="mt-5 rounded-md bg-[#102a2a] px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">{saved ? 'Policy change submitted' : 'Confirm and save policy'}</button></Panel><Panel className="mt-6 overflow-hidden"><div className="border-b border-slate-200 px-5 py-4"><h2 className="font-semibold">Policy history</h2></div><div className="divide-y divide-slate-100">{['v1.2 · 25 Aug 2026 · Risk Manager · Updated verification threshold', 'v1.1 · 12 Aug 2026 · Administrator · Enabled policy', 'v1.0 · 01 Aug 2026 · System · Initial policy'].map((entry) => <p key={entry} className="px-5 py-4 text-sm text-slate-600">{entry}</p>)}</div></Panel></div>; }
function PolicyInput({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label className="text-sm font-semibold">{label}<input type="number" min="0" max="100" value={value} onChange={(event) => onChange(Number(event.target.value))} className="mt-2 block w-full rounded-md border border-slate-300 px-3 py-2.5 font-normal" /></label>; }

export function ModelsPage() { return <div className="mx-auto max-w-7xl px-6 py-8"><PageTitle eyebrow="Intelligence" title="AI Models" detail="Monitor deployed model quality and history. Model changes require model-management access." /><Panel className="p-6"><div className="flex flex-col justify-between gap-4 md:flex-row"><div><p className="text-sm text-slate-500">Currently deployed</p><h2 className="mt-1 text-2xl font-semibold">VeriPay Fraud Detection Model</h2><p className="mt-1 text-sm text-slate-500">Version v1.2 · deployed 18 Aug 2026</p></div><Pill value="DEPLOYED" /></div><div className="mt-6 grid gap-4 sm:grid-cols-3"><Kpi label="AUC" value="0.97" change="+0.02 vs v1.1" tone="teal" /><Kpi label="Precision" value="0.91" change="+0.01 vs v1.1" tone="teal" /><Kpi label="Recall" value="0.88" change="Stable" /></div></Panel><Panel className="mt-6 overflow-hidden"><div className="border-b border-slate-200 px-5 py-4"><h2 className="font-semibold">Model history</h2></div><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-[0.12em] text-slate-500"><tr>{['Model', 'Version', 'AUC', 'Precision', 'Recall', 'Deployment date', 'Status'].map((heading) => <th key={heading} className="px-5 py-3 font-bold">{heading}</th>)}</tr></thead><tbody>{[['VeriPay Fraud Detection', 'v1.2', '0.97', '0.91', '0.88', '18 Aug 2026', 'DEPLOYED'], ['VeriPay Fraud Detection', 'v1.1', '0.95', '0.90', '0.88', '02 Jul 2026', 'RETIRED'], ['VeriPay Fraud Detection', 'v1.0', '0.91', '0.84', '0.81', '14 May 2026', 'RETIRED']].map((row) => <tr key={row[1]} className="border-t border-slate-100">{row.map((cell) => <td key={cell} className="px-5 py-4">{cell === 'DEPLOYED' || cell === 'RETIRED' ? <Pill value={cell} /> : cell}</td>)}</tr>)}</tbody></table></Panel></div>; }

export function ReportsPage() { const [generated, setGenerated] = useState(''); return <div className="mx-auto max-w-5xl px-6 py-8"><PageTitle eyebrow="Reporting" title="Reports" detail="Generate operational and regulatory reports for your institution." /><div className="grid gap-4 md:grid-cols-2">{['Daily fraud report', 'Weekly fraud report', 'Monthly fraud report', 'Fraud prevention report', 'Transaction report', 'Risk report', 'Merchant fraud report'].map((report) => <Panel key={report} className="flex items-center justify-between p-5"><div><h2 className="font-semibold">{report}</h2><p className="mt-1 text-xs text-slate-500">CSV and PDF export available</p></div><button onClick={() => setGenerated(report)} className="rounded-md border border-slate-300 px-3 py-2 text-xs font-semibold hover:bg-slate-50">Generate</button></Panel>)}</div>{generated && <div className="mt-5 rounded-md border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">{generated} generated successfully. Export options are ready.</div>}</div>; }

export function AuditPage() { return <div className="mx-auto max-w-7xl px-6 py-8"><PageTitle eyebrow="Governance" title="Audit Logs" detail="Review administrative actions and access history across the bank console." action={<button className="rounded-md border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold">Export logs</button>} /><Panel className="overflow-hidden"><div className="flex gap-3 border-b border-slate-200 p-5"><input className="flex-1 rounded-md border border-slate-300 px-3 py-2.5 text-sm" placeholder="Filter by user or action..." /><select className="rounded-md border border-slate-300 px-3 py-2.5 text-sm"><option>All resources</option><option>Transaction</option><option>Alert</option><option>Fraud Policy</option></select></div><table className="min-w-[850px] w-full text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-[0.12em] text-slate-500"><tr>{['Timestamp', 'User', 'Role', 'Action', 'Resource', 'Resource ID', 'Result'].map((heading) => <th key={heading} className="px-5 py-3 font-bold">{heading}</th>)}</tr></thead><tbody>{auditLogs.map((row) => <tr key={`${row[0]}-${row[5]}`} className="border-t border-slate-100">{row.map((cell) => <td key={cell} className="px-5 py-4">{cell === 'SUCCESS' ? <Pill value={cell} /> : cell}</td>)}</tr>)}</tbody></table></Panel></div>; }
